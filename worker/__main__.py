...
:func:`ghidra_mcp.ghidra._jvm_bridge.worker_main`.

Env contract (must match ``deploy/worker-run.sh`` + ``rpc-protocol.md`` §2):

- ``GHIDRA_MCP_SESSION_ID`` (required) — the opaque, high-entropy session id. The per-session UDS
  is named ``<sid>.sock`` (BOLA defense: the socket name carries the session identity).
- ``GHIDRA_MCP_RPC_SOCKET_DIR`` (optional, default ``/run/ghidra-mcp``) — the private, owner-only
  directory the ``deploy/`` layer bind-mounts for this session's socket.

The launcher derives ``GHIDRA_MCP_RPC_SOCKET = <dir>/<sid>.sock`` and exports it because that is
...
Fail-closed: a missing/empty ``GHIDRA_MCP_SESSION_ID`` exits non-zero **before** any JVM work, so
...
_DEFAULT_SOCKET_DIR = "/run/ghidra-mcp"
...
def _resolve_socket_path(environ: dict[str, str]) -> str:
    ...
    Raises:
        ValueError: If ``GHIDRA_MCP_SESSION_ID`` is missing or empty (fail closed — the worker
            must not start without a session identity).
    """
    session_id = environ.get("GHIDRA_MCP_SESSION_ID", "").strip()
    if not session_id:
        raise ValueError("GHIDRA_MCP_SESSION_ID is required")
    socket_dir = environ.get("GHIDRA_MCP_RPC_SOCKET_DIR", "").strip() or _DEFAULT_SOCKET_DIR
...
def main(argv: list[str] | None = None) -> int:
    ...
    Exports ``GHIDRA_MCP_RPC_SOCKET`` (the variable
    :func:`ghidra_mcp.ghidra._jvm_bridge.worker_main` reads) and propagates that function's exit
    ...
    try:
        ...
        return 2
    os.environ["GHIDRA_MCP_RPC_SOCKET"] = socket_path
    ...
    from ghidra_mcp.ghidra._jvm_bridge import worker_main
