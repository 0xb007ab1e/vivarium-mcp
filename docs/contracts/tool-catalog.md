...
| `session_import` | `SessionImportIn{source_ref, expected_sha256?, loader?, processor?, base_addr?, entry?, pdb_ref?, debug_ref?, debug_format?, regions?, container?}` | `SessionInfo` | size cap enforced **before** Ghidra; digest verified; path confined (`source_ref` is a path under `VIVARIUM_IMPORT_ROOT`). **Loader hints (ADR-045, F1) are additive + opt-in:** `loader` (`auto`/`binary`, default `auto`) — `auto` drives the opinion/container loaders (ELF/PE) **byte-for-byte as before**; `binary` drives `BinaryLoader` for a **headerless raw/firmware image** and REQUIRES `processor` (a Ghidra `LanguageID` in the allow-list `vivarium.core.languages` — the full installed set, e.g. `ARM:LE:32:Cortex`, `x86:LE:64:default`, `MIPS:BE:32:default`, `RISCV:LE:32:default`) + `base_addr` (image base, bounded to the processor's address width); optional `entry` (entry-point seed, `>= base_addr`). **Hex loaders (ADR-046):** `intel-hex`/`motorola-hex` drive `IntelHexLoader`/`MotorolaHexLoader` for hex-delivered firmware — REQUIRE `processor` only; `base_addr`/`entry` are rejected (the records carry their own addresses). **Self-describing (ADR-047):** `dex`/`macho`/`apk` force `DexLoader`/`MachoLoader`/`ApkLoader` for Android DEX / Mach-O / APK — NO hints allowed (the format carries processor + layout; `auto` also detects these). **Fat-Mach-O slice (ADR-048):** an optional allow-listed `processor` on `loader="macho"` selects that arch slice (via the `program_loader` builder); omit for the default slice. DYLD-component selection is fixture-blocked (deferred). **Companion PDB (ADR-061):** an optional `pdb_ref` (a second confined + size-capped path under `VIVARIUM_IMPORT_ROOT`, `loader="auto"` only) applies a Microsoft PDB's symbols/types to the freshly-loaded PE **before** analysis (Ghidra's cross-platform `pdb2` reader → `DefaultPdbApplicator.applyNoAnalysisState`); a hostile/malformed PDB fails closed `not-found`. Not a write tool (applied at load, no write-consent). **Companion debug map (ADR-071):** an optional `debug_ref` + `debug_format="map"` (a second confined + size-capped path, `loader="auto"` only, mutually exclusive with `pdb_ref`) applies a detached name→address symbol map (linker/`nm`/`.sym` dump) as `IMPORTED` labels to the freshly-loaded ELF **before** analysis; a malformed map fails closed `not-found`, out-of-space addresses are skipped honestly. Not a write tool (applied at load). **Detached DWARF (ADR-071):** `debug_format="dwarf"` (also `loader="auto"` only, mutually exclusive with `pdb_ref`) applies a detached DWARF companion — the worker reads the ELF's `.gnu_debuglink` (pure `core.debuglink`, hostile-byte bounded, name confined to a bare basename — CWE-22), stages the `debug_ref` next to the binary under that name, and Ghidra's DWARF analyzer resolves it (`SameDirDebugInfoProvider`) at analysis time; a binary with no `.gnu_debuglink` fails closed `not-found`. **Multi-region scatter-load (ADR-065):** an optional `regions` list (≤64) drives a headerless raw import into ONE program with N memory blocks — each region `{source_ref | offset+length, base_addr, entry?}` supplies its bytes (its own confined ref, OR a slice of the parent `source_ref`) at its `base_addr`. REQUIRES `loader="binary"` + one shared `processor` (all blocks share the Language — a differing arch is a separate session, ADR-065 D5); mutually exclusive with the top-level single-region `base_addr`/`entry`. Every region is confined + size-capped **before the worker** (per-region CWE-22/CWE-400); overlapping address ranges are rejected server-side (D2). Absent ⇒ byte-for-byte the single-region path. **Container unwrap (ADR-070):** an optional `container` (`gzip`/`xz`/`lzma`/`uimage`) decompresses/unwraps the input BEFORE loading, then hands the payload to the loader hints (auto or binary); `uimage` strips the 64-byte U-Boot legacy header and nested-decompresses its payload under the same caps. Bounded by **two hard zip-bomb caps** — an absolute output ceiling AND an output÷input ratio — the worker STREAMS the decompress and aborts the moment either binds (`limit-exceeded`; a bomb never materializes). Parsed in the WORKER via the stdlib decompressors + the pure `core.uimage` header parser (fuzzed), never the server (ADR-001/D4); a corrupt stream/header fails closed `not-found`. Mutually exclusive with `regions`. Absent ⇒ no-op. (androidboot header format is a tracked follow-up.) Hints are validated **server-side before the worker** (allow-list + width bounds, CWE-20); the worker re-validates the language against the installed set (defense in depth) and fails closed `not-found`. No hint set ⇒ byte-for-byte the pre-ADR-045 path |
...
| `deobfuscate_strings` | `DeobfuscateStringsIn{function?, techniques?=[stack_string, xor_decode], min_length, max_results≤10000, max_bytes≤10000, max_steps?}` | `DeobfuscateStringsOut{strings[]{address, technique, text*, length, encoding*?, decode_key*?}, truncated}` | **ADR-068** recover hidden strings (read-only). The `stack_string` technique walks a function's RAW per-instruction p-code (NOT the decompiler's `HighFunction` — dead-code elimination removes stores never read, which is how these hide) for runs of constant stores to adjacent stack slots (`INT_ADD(frame/stack reg, const)` addr + `COPY(const)`/direct value + `STORE`), reassembling contiguous slots (NUL-split, printable-ratio + `min_length` filter) via the pure `core.stackstring`. Pure static analysis — no execution. `function`-scoped or a bounded whole-program scan; bounded by `max_results`/`max_bytes` + a function-scan cap; `truncated` honest. The `xor_decode` technique (ADR-068 D3) detects an in-place XOR-with-const decode loop (INT_XOR + STORE + a write data-ref) and emulates it in the ADR-049 sandbox (`max_steps`-bounded) — reporting a run only if it BECAME printable, with `encoding`/`decode_key` (both `*`=UNTRUSTED); `add`-decoders + multi-stage/keyed are a tracked follow-up. Recovered `text`/`encoding`/`decode_key` are `*`=UNTRUSTED (binary-derived); address/technique/length SAFE |
...
| `emulate` | `EmulateIn{start, set_registers?, write_memory?, max_steps≤1M, stop_at?, read_registers?, read_memory?, call?, args?, stubs?}` | `EmulateOut{steps_executed, stop_reason=stop-address\|max-steps\|halted\|fault\|stub-limit, registers[]{name, value*}, memory[]{address, data*, length}, return_value*}` | **ADR-049 p-code emulation** (read-effect-only). Ghidra's p-code **interpreter** — NO native execution / syscalls / I/O; program DB not mutated. Bounded by `max_steps` (server-clamped, default 100k) + the per-call wall-clock kill + worker memory cap. Register/memory readback VALUES are `*`=UNTRUSTED (attacker-influenced). All parsing/emulation stays in the ephemeral worker container. **Call convenience (ADR-066):** `call=true` treats `start` as a function to *call* — the worker sets up a scratch stack + a sentinel return (writing it at the stack pointer and, on link-register arches, into LR), runs to the return (implicit `stop_at`), and reads the ABI return register into `return_value*`. Args + input buffers stay caller-provided via `set_registers`/`write_memory` (the raw calling convention is NOT auto-resolved — proven unreliable), outputs via `read_memory`; automates only the sentinel-return + return-value dance. **Auto arg-placement (ADR-066):** with `call=true`, an optional `args` (list of ints) is placed into the target function's parameter storage per its RESOLVED calling convention (register params only; a stack-param convention fails closed; `args` without `call` is rejected). **Library-call stubs (ADR-066 D2):** `stubs=[{target(addr\|name), action}]` where action is `return_const:<int>` (set the ABI return register + continue past the call) or `skip` (no-op) — substitutes an external call (`memcpy`/`strlen`/a ROM thunk) so a self-contained routine completes instead of halting; a stub NEVER runs real code or touches the host (the interpreter still makes no syscalls); an unstubbed unresolved call still halts (opt-in); the stub-application budget stopping it yields `stop_reason=stub-limit`. Auto arg-placement from a resolved prototype is a tracked follow-up |
...
| `search_bytes` | `SearchBytesIn{pattern_hex, offset, limit}` | `SearchBytesOut{matches[]{address, context_hex*}, total, truncated}` | BOUNDED byte-pattern search — hex pattern with optional `??` wildcards (validated, length-bounded); each match's surrounding `context_hex` is `*`=UNTRUSTED (binary-derived), address/total/truncated safe |
...
| `crypto_detect` | `CryptoDetectIn{offset, limit, min_length}` | `CryptoDetectOut{indicators[{address?, kind, source, detail*, confidence}], total, truncated}` | **ADR-075** crypto detection (read-only, heuristic) — **complements, does NOT replace, `crypto_constant_scan`**. Detects crypto by an imported crypto **API** (`import` source: CryptoAPI/CNG/CommonCrypto/OpenSSL/libsodium/mbedTLS symbols; pure `core.cryptodetect` over `list_imports`), a resolved crypto **symbol NAME** in the strings (`api_name` source, symbol-like only, over `list_strings`), and a **hardware crypto opcode** (`instruction` source: AES-NI / SHA-ext / `pclmulqdq`, via the worker `crypto_instructions` opcode scan). Fixes the framework-crypto blind spot the constant scan misses (validation miss M2 — e.g. CommonCrypto AES). `kind`∈{aes,sha,md5,rc4,hmac,crypto_api}, `source`∈{import,api_name,instruction}, `confidence` per source (import/instruction 0.9, api_name 0.6); `detail`(`*`=UNTRUSTED matched symbol/mnemonic). HEURISTIC — empty ≠ no crypto. **MVP:** the `code_pattern` (cipher-shaped loops) source remains a tracked fast-follow |
| `capability_scan` | `CapabilityScanIn{offset, limit}` | `CapabilityScanOut{capabilities[{rule_id, name, namespace, attack[{tactic, technique_id}], evidence[{address?, where, detail*}], confidence}], total, truncated, rule_pack_version}` | **ADR-074** capa-style capability detection mapped to **MITRE ATT&CK** (read-only, heuristic). Pure core (`core.capabilityscan`) over `list_imports` + `list_exports` + `list_strings` — a curated built-in rule pack (`rule_pack_version`, all/any/n_of match modes) emits named capabilities each with ATT&CK technique id(s) + evidence: regsvr32 exec (T1218.010), reflective load (T1620), process injection (T1055), named-pipe C2 (T1559.001), anti-debug (T1622), Run-key persistence (T1547.001), keylog (T1056.001), screen capture (T1113), browser creds (T1555.003), sniffing (T1040), network C2 (T1071), command shell (T1059), host recon (T1082). Evidence `detail`(`*`=UNTRUSTED); rule_id/name/namespace/attack/confidence SAFE. HEURISTIC — empty ≠ benign on a packed input (real capabilities live in the encoded stage). **MVP:** the full external capa-rules pack (signed, D2) + disassembly rules are a tracked fast-follow |
