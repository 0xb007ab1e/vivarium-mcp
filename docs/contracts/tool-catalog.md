...
| `emulate` | `EmulateIn{start, set_registers?, write_memory?, max_steps≤1M, stop_at?, read_registers?, read_memory?, call?}` | `EmulateOut{steps_executed, stop_reason, registers[]{name, value*}, memory[]{address, data*, length}, return_value*}` | **ADR-049 p-code emulation** (read-effect-only). Ghidra's p-code **interpreter** — NO native execution / syscalls / I/O; program DB not mutated. Bounded by `max_steps` (server-clamped, default 100k) + the per-call wall-clock kill + worker memory cap. Register/memory readback VALUES are `*`=UNTRUSTED (attacker-influenced). All parsing/emulation stays in the ephemeral worker container. **Call convenience (ADR-066):** `call=true` treats `start` as a function to *call* — the worker sets up a scratch stack + a sentinel return (writing it at the stack pointer and, on link-register arches, into LR), runs to the return (implicit `stop_at`), and reads the ABI return register into `return_value*`. Args + input buffers stay caller-provided via `set_registers`/`write_memory` (the raw calling convention is NOT auto-resolved — proven unreliable), outputs via `read_memory`; automates only the sentinel-return + return-value dance. Auto arg-placement from a resolved prototype + library-call stubs are tracked follow-ups |
