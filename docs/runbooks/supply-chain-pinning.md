...
## Phase 1 — Pin the 3 base images by digest (4 occurrences)
...
# 1c. python:3.12-slim  (Containerfile.server builder + runtime — 2 occurrences)
...
# 1d. Rewrite the placeholders (exact-match sed; @<placeholder> -> @<sha256:...>):
...
sed -i "s|python:3.12-slim@REPLACE_WITH_DIGEST_FOR_python:3.12-slim|python:3.12-slim@${PY_DIGEST}|g" Containerfile.server

# 1e. Confirm no base-image placeholder remains:
grep -n "REPLACE_WITH_DIGEST_FOR_eclipse\|REPLACE_WITH_DIGEST_FOR_python" Containerfile.* || echo "base images pinned ✓"
```
