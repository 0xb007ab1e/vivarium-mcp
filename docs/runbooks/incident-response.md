...
- Incident channel `<#channel>`; paging; access to server + worker logs and the container runtime;
...
3. **Preserve evidence FIRST** on a suspected escape: capture worker logs + `<container inspect/diff>`
...
## Post-incident
- Blameless post-mortem within `<N>` days: timeline, root cause, tracked action items. Feed gaps
