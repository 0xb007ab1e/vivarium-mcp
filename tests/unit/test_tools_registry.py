...
def test_catalog_is_exactly_67_unique_tools() -> None:
    ...
    # 1 whole-program fingerprint (ADR-073 D1: program_fingerprint; read-only) + 1 crypto detection
    # by API/import/symbol (ADR-075: crypto_detect; read-only) + 1 capability detection + ATT&CK
    # tagging (ADR-074: capability_scan; read-only) + 1 offline family match by fingerprint
    # (ADR-073 D2: family_match; read-only) —
    ...
    assert len(reg.TIER1_TOOL_NAMES) == 78
    assert len(set(reg.TIER1_TOOL_NAMES)) == 78
