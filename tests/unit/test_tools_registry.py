...
def test_catalog_is_exactly_50_unique_tools() -> None:
    ...
    # multi-type composite batch (ADR-021: define_types, GATED by allow_structural) — the 13
    # mutation tools GATED by per-session write-consent (the structural 7 additionally by
    # allow_structural); import is GATED identically (+ allow_structural for structural entries).
    assert len(reg.TIER1_TOOL_NAMES) == 50
    assert len(set(reg.TIER1_TOOL_NAMES)) == 50
