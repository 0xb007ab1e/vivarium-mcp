"""Integration: p-code (IR) listing runs against a real worker (ADR-052, v1.8 get_pcode).

Regression + capability gate for the ``get_pcode`` tool: Ghidra lifts each machine instruction to
its low p-code (IR) via ``Instruction.getPcode()`` — the SAME IR the ``emulate`` interpreter steps —
without executing anything or touching the program DB (read-only). Proven live:

    * import a tiny x86-64 blob (``mov eax,5 ; add eax,3 ; ret``), disassemble it, and ``get_pcode``
      over the range yields per-instruction ops: ``mov`` → a ``COPY``, ``add`` → an ``INT_ADD``,
      ``ret`` → a ``RETURN``.

Why a gated in-container test (not a unit test): ``Instruction.getPcode`` is the JVM/PyGhidra edge
...
#: Generous ceiling for JVM boot + import + disassemble + the p-code lift.
...
# Runs as `python -c <DRIVER>` INSIDE the worker image: import a tiny x86-64 blob, disassemble the
# entry, then call `get_pcode` over the range and return the per-instruction p-code. Ends with an
# explicit flush + os._exit(0) so a lingering JVM thread never blocks interpreter shutdown.
_DRIVER = r"""
...
def main():
    ...
    path = os.path.join(project_dir, "pcode.bin")
    ...
    # Disassemble the entry so the listing has instructions for get_pcode to lift.
    from ghidra.program.flatapi import FlatProgramAPI
    fpa = FlatProgramAPI(program)
    addr = program.getAddressFactory().getDefaultAddressSpace().getAddress(base)
    tx = program.startTransaction("disasm")
    fpa.disassemble(addr)
    program.endTransaction(tx, True)

    return backend.get_pcode({"start": hex(base), "function": None, "max_instructions": 16})
...
def _build_command(engine: str, image: str) -> list[str]:
    """Build the container-run argv that drives the p-code listing in ``image``.
...
def test_get_pcode_lifts_instructions_on_real_worker(worker_image: str) -> None:
    """``get_pcode`` must lift each instruction to its low p-code ops (ADR-052).

    Drives the in-container backend: import a blob, disassemble it, then ``get_pcode`` over the
    range. Asserts the lifted ops match the known lifting — ``mov`` → a ``COPY``, ``add`` → an
    ``INT_ADD``, ``ret`` → a ``RETURN`` — and that mnemonics are present. A regression re-surfaces
    as ``ok=False`` or missing ops.
    ...
    try:
        ...
        pytest.fail(
            f"get_pcode run timed out after {_RUN_TIMEOUT_SECONDS}s "
    ...
    assert envelope.get("ok") is True, (
        f"get_pcode reported failure (ADR-052 regression?): {envelope.get('error')!r}\n"
        f"{envelope.get('traceback', '')[-2000:]}"
    )
    data = envelope["data"]

    instructions = data.get("instructions", [])
    assert len(instructions) >= 3, f"expected >=3 lifted instructions; got {instructions!r}"

    mnemonics = {str(i.get("mnemonic")) for i in instructions}
    assert {"MOV", "ADD", "RET"} <= mnemonics, f"expected MOV/ADD/RET; got {mnemonics!r}"

    all_ops = " ".join(op for i in instructions for op in i.get("pcode", []))
    assert "COPY" in all_ops, f"expected a COPY op (mov lift); got {all_ops!r}"
    assert "INT_ADD" in all_ops, f"expected an INT_ADD op (add lift); got {all_ops!r}"
    assert "RETURN" in all_ops, f"expected a RETURN op (ret lift); got {all_ops!r}"
