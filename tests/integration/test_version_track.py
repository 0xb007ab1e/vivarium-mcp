"""Integration: two-program Version Tracking on a real worker (ADR-060, v1.8 version_track).

Regression + capability gate for the ``version_track`` tool: Ghidra VT correlates functions
between TWO binaries loaded fresh in one worker. Proven live:

    * write two IDENTICAL synthetic ARM ELFs; ``version_track(a, b,
      correlator="exact_instructions")`` returns at least one function match, and the matched
      function scores ``similarity == 1.0`` (identical code) with ``source_address ==
      destination_address`` (same layout).

Why a gated in-container test (not a unit test): the VT pipeline (two fresh domain-file loads +
two auto-analyses + the correlator over the loaded address sets) is the JVM/PyGhidra edge (TB3,
ADR-001) — excluded from server unit coverage and only validatable against a real Ghidra worker.
Gating + fixture posture are reused verbatim from ``test_find_similar_functions.py``:
...
Honored environment (same as ``test_find_similar_functions.py``):
...
#: Generous ceiling for JVM boot + TWO imports + TWO auto-analyses + the correlation.
...
#: Ghidra analyzes it into ``ARM:LE:32:v8`` with one function; two copies exact-instructions-match.
_ARM_ELF_HEX = (
...
# Runs as `python -c <DRIVER>` INSIDE the worker image: write two identical ELFs, then
# version_track them with the exact-instructions correlator. Ends with an explicit flush +
# os._exit(0) — VT's non-daemon JVM threads otherwise keep the process alive and lose buffered
# stdout.
_DRIVER = r"""
...
def main():
    ...
    return backend.version_track(
        {
            "source_ref_a": path_a,
            "source_ref_b": path_b,
            "correlator": "exact_instructions",
            "min_confidence": 0.0,
            "limit": 50,
        }
    )
...
def _build_command(engine: str, image: str) -> list[str]:
    """Build the container-run argv that drives the VT correlation in ``image``.

    Mirrors ``test_find_similar_functions.py``'s posture exactly (network-isolated, capped memory,
...
def test_version_track_matches_identical_programs_on_real_worker(worker_image: str) -> None:
    """``version_track`` must correlate the shared function between two identical programs.

    Drives the in-container backend: write two identical synthetic ARM ELFs, then
    ``version_track(a, b, correlator="exact_instructions")``. Asserts at least one match, a
    ``match_count >= 1``, and that some returned match scores ``similarity == 1.0`` with
    ``source_address == destination_address`` (identical code + layout). A regression re-surfaces as
    ``ok=False`` (e.g. the VT lock lifecycle breaking) or an empty match set.
    ...
    try:
        ...
        pytest.fail(
            f"version_track run timed out after {_RUN_TIMEOUT_SECONDS}s "
    ...
    assert envelope.get("ok") is True, (
        f"version_track reported failure (ADR-060?): {envelope.get('error')!r}\n"
        f"{envelope.get('traceback', '')[-2000:]}"
    )
    data = envelope["data"]

    assert int(data["match_count"]) >= 1, f"expected at least one VT match; got {data!r}"
    matches = data["matches"]
    assert matches, f"expected a non-empty match list; got {data!r}"
    identical = [
        m
        for m in matches
        if float(m["similarity"]) == 1.0 and m["source_address"] == m["destination_address"]
    ]
    assert identical, f"expected an identical-code match (sim 1.0, same address); got {matches!r}"
