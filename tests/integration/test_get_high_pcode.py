"""Integration: high (SSA) p-code runs against a real worker (ADR-053, v1.8 get_high_pcode).

Regression + capability gate for the ``get_high_pcode`` tool: the decompiler's REFINED high p-code —
SSA, dead-code-eliminated, constant-folded — via ``HighFunction.getPcodeOps()``. It shows what a
function semantically does, not the raw per-instruction lifting. Proven live:

    * import ``mov eax,5 ; add eax,3 ; ret``, create a function over it, ``get_high_pcode`` → the
      decompiler folds ``5 + 3`` to a single ``COPY 0x8`` (+ a ``RETURN``) — far fewer ops than the
      ~14 raw ops ``get_pcode`` (low p-code) returns for the same code.

Why a gated in-container test (not a unit test): ``DecompInterface`` + ``HighFunction`` are the
JVM/PyGhidra edge (TB3, ADR-001) — excluded from server unit coverage and only validatable against a
real Ghidra worker. Gating + fixture posture are reused verbatim from ``test_import_emulate.py``:
``integration``-marked (the default unit run SKIPS it), runs only when ``VIVARIUM_INTEGRATION`` is
truthy AND a real worker image + engine are available. No real malware — the input is a tiny
synthetic x86-64 blob.
...
#: Generous ceiling for JVM boot + import + decompile.
...
# Runs as `python -c <DRIVER>` INSIDE the worker image: import a tiny x86-64 blob, create a function
# over it, then call `get_high_pcode` and return its refined ops. Ends with an explicit flush +
# os._exit(0) so a lingering JVM thread never blocks interpreter shutdown.
_DRIVER = r"""
...
def main():
    ...
    path = os.path.join(project_dir, "high.bin")
    with open(path, "wb") as handle:
        handle.write(b"\xb8\x05\x00\x00\x00\x83\xc0\x03\xc3")  # mov eax,5 ; add eax,3 ; ret
    ...
    fpa.createFunction(addr, "compute")
    program.endTransaction(tx, True)

    return backend.get_high_pcode({"function": "compute", "max_ops": 64})
...
def _build_command(engine: str, image: str) -> list[str]:
    """Build the container-run argv that drives the high-p-code listing in ``image``.
...
def test_get_high_pcode_returns_refined_ir_on_real_worker(worker_image: str) -> None:
    """``get_high_pcode`` must return the decompiler's optimized SSA IR (ADR-053).

    Drives the in-container backend: import a blob, create a function, then ``get_high_pcode``.
    Asserts the refined IR reflects the decompiler optimization — a constant-folded ``COPY`` of the
    ``5 + 3 = 8`` result and a ``RETURN`` — with far fewer ops than the raw low p-code. A regression
    re-surfaces as ``ok=False`` or the optimization not appearing.
    ...
    try:
        ...
        pytest.fail(
            f"get_high_pcode run timed out after {_RUN_TIMEOUT_SECONDS}s "
    ...
    assert envelope.get("ok") is True, (
        f"get_high_pcode reported failure (ADR-053 regression?): {envelope.get('error')!r}\n"
        f"{envelope.get('traceback', '')[-2000:]}"
    )
    data = envelope["data"]

    ops = data.get("ops", [])
    assert ops, f"expected >=1 high p-code op; got {ops!r}"
    # High p-code is heavily optimized — the whole 3-instruction body collapses to a handful of ops
    # (vs. ~14 raw low-p-code ops), proving this is the refined IR, not the raw lifting.
    assert len(ops) <= 8, f"expected the optimized SSA IR (few ops); got {len(ops)}: {ops!r}"

    op_texts = " ".join(str(o.get("op", "")) for o in ops)
    assert "COPY" in op_texts, f"expected a constant-folded COPY op; got {op_texts!r}"
    assert "0x8" in op_texts, f"expected the folded 5+3=8 constant (0x8); got {op_texts!r}"
    assert "RETURN" in op_texts, f"expected a RETURN op; got {op_texts!r}"
