...
and a per-function :class:`~vivarium.tools.schemas.FunctionContext` bundle, walk the functions
...
def _assemble(results: list[FunctionNaming]) -> str:
    ...
    header = "/* vivarium semantic-naming reference output (best-effort; NOT a guarantee) */\n"
    parts = [header]
    if extern_decls:
        parts.append("\n".join(extern_decls))
    parts.extend(bodies)
