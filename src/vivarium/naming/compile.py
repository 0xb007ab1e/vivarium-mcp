...
:mod:`vivarium.naming.metrics`, so the pure eval scorer stays runtime-agnostic. The subprocess
...
bounded ``(exit_code, stdout)`` into a :class:`~vivarium.naming.metrics.RunResult`. It satisfies
...
from vivarium.naming.metrics import CompileResult, RunResult
...
class ContainerCompileRunner:
    """A :data:`~vivarium.naming.metrics.CompileRunner` that compiles untrusted C in isolation.
...
class ContainerExecRunner:
    """An :data:`~vivarium.naming.metrics.ExecRunner`: build + run untrusted C in isolation.
