"""Process entry point for the vivarium server (stdio transport, v1).
...
from vivarium.config import Config, load_config
from vivarium.core.errors import GhidraMcpError
from vivarium.ghidra.port import GhidraPort
from vivarium.logging import configure_logging, get_logger
from vivarium.server.app import build_app, run_http, run_stdio
...
def _default_port_factory(config: Config) -> GhidraPort:
    ...
    Returns:
        A :class:`vivarium.ghidra.port.GhidraPort` implementation.
    """
    from vivarium.ghidra.launcher import ContainerWorkerLauncher, make_confined_resolver
...
def _default_session_manager_factory(config: Config, port: GhidraPort) -> SessionManager:
    ...
    Returns:
        A constructed :class:`vivarium.sessions.manager.SessionManager`.
    """
