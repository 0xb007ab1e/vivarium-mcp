...
- :class:`AuthenticationMiddleware` — default-deny: runs the injected :class:`Authenticator`
  (`vivarium.server.auth`); a rejected request gets a generic ``401`` (no oracle), an accepted one
  has its :class:`~vivarium.server.auth.Principal` stashed on the ASGI scope for the per-request
  authZ check (slice 4). It builds the :class:`~vivarium.server.auth.AuthContext` from the
...
frozen :class:`~vivarium.core.errors.ErrorEnvelope` shape and leak nothing.
"""
...
#: (:func:`vivarium.server.app._http_principal_resolver`) reads the same key.
SCOPE_PRINCIPAL_KEY = "vivarium.principal"
