...
the client-CA bundle) and the in-app :class:`~vivarium.server.auth.MtlsAuthenticator`, but left
...
- :class:`MtlsAwareProtocol` — a thin subclass of uvicorn's h11 HTTP protocol that, **at scope
  ...
  (:mod:`vivarium.server.http_middleware`) reads into
  :attr:`~vivarium.server.auth.AuthContext.peer_certificate`. The cert is sourced
...
def build_tls_scope_extension(ssl_object: ssl.SSLObject | None) -> dict[str, Any] | None:
    ...
    is the shape :class:`~vivarium.server.auth.MtlsAuthenticator` expects.
...
class MtlsAwareProtocol(H11Protocol):  # pragma: no cover - real TLS socket; covered by integration
    ...
    Used **only** when ``auth_mode == "mtls"`` (wired in :func:`vivarium.server.app.run_http`);
    ...
    :func:`vivarium.server.http_middleware._peer_certificate` reads, with no copy of uvicorn's
