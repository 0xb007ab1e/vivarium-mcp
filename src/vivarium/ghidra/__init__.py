...
(:mod:`vivarium.ghidra.port`) that the session manager and tools depend on, and an **adapter**
(:mod:`vivarium.ghidra.rpc_client`) that spawns/kills hardened worker containers and speaks the
...
parses a binary. The JVM bridge (:mod:`vivarium.ghidra._jvm_bridge`) runs ONLY inside the worker
